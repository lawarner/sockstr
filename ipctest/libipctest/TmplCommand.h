...
protected:
    BasicCommand(void) { };
    BasicCommand(const std::string& cname, void* data = 0, int delay = 0)
        : commandName_(cname), data_(data), delay_(delay) {  }

private:
    BasicCommand(const BasicCommand&);	// disable copy constructor
    BasicCommand& operator=(const BasicCommand& rSource);	// disable assignment operator

protected:
    std::string commandName_;	// Name of command
    DATATYPE data_;	// any data related to command
